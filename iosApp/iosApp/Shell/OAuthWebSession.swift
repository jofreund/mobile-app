...
///
/// Named for the mechanism rather than the role because `OAuthHandler` is the exported
/// Obj-C protocol from the Kotlin framework — a class of that name in this target would
/// shadow it and break conformance.
///
/// Staying in-app is the point. The previous implementation handed the URL to Safari via
/// `UIApplication.openURL`, which backgrounded the app; returning by deep link then
/// foregrounded it and triggered a transport reconnect that raced the token delivery. A
/// session keeps the app foregrounded and the websocket connected for the whole flow, and
/// it uses the Safari engine no matter which browser the user has set as default — Chrome
/// silently blocks the JS-initiated custom-scheme redirect this flow depends on.
final class OAuthWebSession: NSObject, OAuthHandler, ASWebAuthenticationPresentationContextProviding {
    ...
    /// also infer abandonment from foreground events (see `AuthenticationManager`'s
    /// `foregroundEvents` collector).
    ...
    /// `UIApplicationSupportsMultipleScenes` is true, so filter by type and activation
    /// rather than taking the first connected scene.
