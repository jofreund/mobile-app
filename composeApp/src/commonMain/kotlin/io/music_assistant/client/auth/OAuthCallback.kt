 ...
 *
 * Two callers share it: the in-app auth session and the deep-link dispatcher in
 * `iOSApp.swift`. They used to hold separate copies of the scheme, which could drift
 * apart silently — a callback scheme that does not match the one the session waits for
 * produces a sheet that hangs until the user gives up.
 */
object OAuthCallback {
     ...
     *
     * Must stay in step with the `CFBundleURLSchemes` entry in `iosApp/iosApp/Info.plist`,
     * which is what makes iOS deliver the callback to this app at all.
     */
     ...
     *
     * A URL that matches the shape but carries `?error=` returns [OAuthCallbackResult.Failed]
     * rather than being dropped — the user is sitting in front of a spinner that only an
     * outcome can clear.
     */
