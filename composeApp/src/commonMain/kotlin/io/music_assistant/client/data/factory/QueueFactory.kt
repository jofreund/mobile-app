...
class QueueFactory(
    ...
    fun create(server: ServerQueue): QueueInfo = with(server) {
        QueueInfo(
            ...
            supportsAutoplayCommand = autoplayEnabled != null,
     ...
     *    provider. A queued track otherwise resolves nothing on 2.10: `Track.image` is a
     *    server-side property (never serialized), `metadata.images` is null, and the
     *    flattened `album` ItemMapping usually carries `image: null` too.
     *
