...
/**
 * The single definition of what an OAuth callback URL looks like, shared by the in-app
 * auth session and the deep-link handler in `iOSApp.swift`. A disagreement between those
 * two used to be possible, and shows up as a login that hangs.
 */
class OAuthCallbackTest {
    ...
    @Test
    fun `the scheme matches the one registered in Info plist`() {
        // iOS only delivers the callback to this app because CFBundleURLSchemes lists it.
        // A change here without a matching Info.plist edit produces a session that never
        // returns, so pin the literal rather than deriving it.
        assertEquals("musicassistant", OAuthCallback.SCHEME)
    }
    ...
    fun `a provider error is reported instead of dropped`() {
        val result = OAuthCallback.parse("musicassistant://auth/callback?error=access_denied")
    ...
    @Test
    fun `error description is preferred over the error code`() {
    ...
    @Test
    fun `a trailing slash still matches the callback path`() {
        val result = OAuthCallback.parse("musicassistant://auth/callback/?code=abc123")
